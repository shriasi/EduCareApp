...
public class WelcomeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try{
            if(!prefs.contains("ad_points")){
                editor.putInt("ad_points",0);
                editor.apply();
            }
        }catch (Exception er){
            Log.d("scanimated", "Shared prefs error:" + er.toString());
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == RC_SIGN_IN) {
            ...
            try {
                ...
                Toast.makeText(WelcomeActivity.this, "Sign In Success!",
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            if(!prefs.contains("username")){
                                editor.putString("username", Objects.requireNonNull(user).getDisplayName());
                                editor.putString("id",user.getUid());
                                editor.putString("email",user.getEmail());
                                editor.putString("avatar", Objects.requireNonNull(user.getPhotoUrl()).toString());
                                editor.apply();
                            }
